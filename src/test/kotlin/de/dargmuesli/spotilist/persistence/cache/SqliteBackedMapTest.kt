...
class SqliteBackedMapTest {
    @Test
    fun `entries survive a commit and a fresh connection to the same file`(@TempDir tempDir: Path) {
        val file = tempDir.resolve("cache.db")

        DriverManager.getConnection("jdbc:sqlite:$file").use { connection ->
            val map = SqliteBackedMap(connection, "playlists", encode = { it }, decode = { it })
